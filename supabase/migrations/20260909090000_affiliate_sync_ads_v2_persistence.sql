-- A11-S4-I2 closed coded-Impact-Ads persistence contract.
-- Extends the settled V2 transaction/replay boundary without changing the
-- checkpointed I1 migration or the historical V1 public.import_apply function.
...
-- Namespace uniqueness keeps Promotion and Ad identities independent, while
-- an unproven legacy identity must remain a hard collision for either proven
-- namespace. Fail closed before index creation if unexpected historical rows
-- already violate that invariant.
DO $impact_legacy_namespace_guard$
BEGIN
  IF EXISTS (
    SELECT 1
    FROM public.stores AS legacy_store
    JOIN public.stores AS campaign_store
      ON campaign_store.provider = legacy_store.provider
     AND campaign_store.provider_entity_id = legacy_store.provider_entity_id
    WHERE legacy_store.provider = 'impact'
      AND legacy_store.provider_entity_namespace = 'legacy'
      AND campaign_store.provider_entity_namespace = 'campaign'
  ) THEN
    RAISE EXCEPTION 'existing_impact_store_legacy_namespace_collision'
      USING ERRCODE = '23505';
  END IF;

  IF EXISTS (
    SELECT 1
    FROM public.coupons AS legacy_offer
    JOIN public.coupons AS proven_offer
      ON proven_offer.provider = legacy_offer.provider
     AND proven_offer.provider_entity_id = legacy_offer.provider_entity_id
    WHERE legacy_offer.provider = 'impact'
      AND legacy_offer.provider_entity_namespace = 'legacy'
      AND proven_offer.provider_entity_namespace IN ('promotion', 'ad')
  ) THEN
    RAISE EXCEPTION 'existing_impact_offer_legacy_namespace_collision'
      USING ERRCODE = '23505';
  END IF;
END;
$impact_legacy_namespace_guard$;

-- These indexes make the invariant atomic for every writer, not only callers
-- of the V2 persistence RPC. Separate coupon guards intentionally allow an Ad
-- and a Promotion with the same literal provider ID to coexist.
CREATE UNIQUE INDEX stores_impact_campaign_legacy_identity_uidx
  ON public.stores (provider, provider_entity_id)
  WHERE provider = 'impact'
    AND provider_entity_namespace IN ('campaign', 'legacy')
    AND provider_entity_id IS NOT NULL;

CREATE UNIQUE INDEX coupons_impact_ad_legacy_identity_uidx
  ON public.coupons (provider, provider_entity_id)
  WHERE provider = 'impact'
    AND provider_entity_namespace IN ('ad', 'legacy')
    AND provider_entity_id IS NOT NULL;

CREATE UNIQUE INDEX coupons_impact_promotion_legacy_identity_uidx
  ON public.coupons (provider, provider_entity_id)
  WHERE provider = 'impact'
    AND provider_entity_namespace IN ('promotion', 'legacy')
    AND provider_entity_id IS NOT NULL;

-- Preserve the exact reviewed Promotions implementation behind a private
-- dispatcher target. The public signature is recreated below.
ALTER FUNCTION public.apply_affiliate_persistence_plan_v2(
  uuid, text, text, text, text, timestamptz, uuid, jsonb, jsonb, jsonb
) RENAME TO affiliate_sync_v2_apply_promotions_plan_internal;

REVOKE ALL ON FUNCTION public.affiliate_sync_v2_apply_promotions_plan_internal(
  uuid, text, text, text, text, timestamptz, uuid, jsonb, jsonb, jsonb
...
BEGIN
  ...
  BEGIN
    IF _integration_id IS NULL
      ...
      OR _persistence_contract_version IS DISTINCT FROM 'v2-a11-ads-1'
    ...
    LOOP
      ...
      v_legacy_collision_id := NULL;

      SELECT store.id INTO v_legacy_collision_id
      FROM public.stores AS store
      WHERE store.provider = 'impact'
        AND store.provider_entity_namespace = 'legacy'
        AND store.provider_entity_id = v_provider_entity_id
      FOR SHARE;

      IF v_legacy_collision_id IS NOT NULL THEN
      ...
      IF v_action = 'create' THEN
        ...
        IF v_existing_store_id IS NULL THEN
          ...
          EXCEPTION WHEN unique_violation THEN
            ...
            IF v_unique_constraint_name NOT IN (
              'stores_slug_key',
              'stores_impact_campaign_legacy_identity_uidx'
            ) THEN
              RAISE;
            END IF;
            v_entity_id := NULL;
    ...
    LOOP
      ...
      SELECT offer.id INTO v_legacy_collision_id
      FROM public.coupons AS offer
      WHERE offer.provider = 'impact'
        AND offer.provider_entity_namespace = 'legacy'
      ...
      IF v_action = 'create' THEN
        IF v_existing_offer_id IS NULL THEN
          ...
          EXCEPTION WHEN unique_violation THEN
            GET STACKED DIAGNOSTICS
              v_unique_constraint_name = CONSTRAINT_NAME;
            IF v_unique_constraint_name IS DISTINCT FROM
              'coupons_impact_ad_legacy_identity_uidx'
            THEN
              RAISE;
            END IF;
            v_entity_id := NULL;
        ...
        ELSE
          ...
          END IF;
          IF v_existing_offer_kind IS DISTINCT FROM 'code' THEN
