-- Affiliate Sync V2 A9C-S2: replay-safe, create/no-op-only persistence.
--
-- This migration is additive. It does not rewrite historical import runs,
-- alter V1 import_apply, or change the existing catalog identity indexes.

CREATE OR REPLACE FUNCTION public.affiliate_sync_v2_has_exact_keys(
  _value jsonb,
  _keys text[]
)
RETURNS boolean
...
DECLARE
  v_key_count integer;
BEGIN
  IF jsonb_typeof(_value) IS DISTINCT FROM 'object' OR _keys IS NULL THEN
    RETURN false;
  END IF;
  SELECT count(*) INTO v_key_count FROM jsonb_object_keys(_value);
  RETURN v_key_count = cardinality(_keys) AND _value ?& _keys;
EXCEPTION WHEN OTHERS THEN
  RETURN false;
END;
$function$;

CREATE OR REPLACE FUNCTION public.affiliate_sync_v2_is_nonnegative_integer(
  _value jsonb
)
RETURNS boolean
LANGUAGE plpgsql
...
DECLARE
  v_number numeric;
BEGIN
  IF jsonb_typeof(_value) IS DISTINCT FROM 'number' THEN
    RETURN false;
  END IF;
  v_number := (_value #>> '{}')::numeric;
  RETURN v_number >= 0
    AND trunc(v_number) = v_number
    AND v_number <= 2147483647;
EXCEPTION WHEN OTHERS THEN
  RETURN false;
END;
$function$;

CREATE OR REPLACE FUNCTION public.affiliate_sync_v2_is_iso_date(
  _value text
)
RETURNS boolean
LANGUAGE plpgsql
...
AS $function$
BEGIN
  IF _value IS NULL OR _value !~ '^[0-9]{4}-[0-9]{2}-[0-9]{2}$' THEN
    RETURN false;
  END IF;
  RETURN (_value::date)::text = _value;
EXCEPTION WHEN OTHERS THEN
  RETURN false;
END;
$function$;

CREATE OR REPLACE FUNCTION public.affiliate_sync_v2_is_canonical_provider_id(
  _value text
)
RETURNS boolean
LANGUAGE sql
...
AS $function$
  SELECT _value IS NOT NULL
    AND _value <> ''
    AND _value = pg_catalog.btrim(
      _value,
      U&'\0009\000A\000B\000C\000D\0020\00A0\1680\2000\2001\2002\2003\2004\2005\2006\2007\2008\2009\200A\2028\2029\202F\205F\3000\FEFF'
    );
$function$;

CREATE OR REPLACE FUNCTION public.affiliate_sync_v2_valid_expected_counts(
  _counts jsonb
)
RETURNS boolean
LANGUAGE plpgsql
...
BEGIN
  IF NOT public.affiliate_sync_v2_has_exact_keys(
      _counts,
      ARRAY['stores', 'offers', 'writableStores', 'writableOffers', 'writableEntities']
    )
    OR NOT public.affiliate_sync_v2_has_exact_keys(
      _counts->'stores',
      ARRAY['create', 'noopExisting', 'blockedAmbiguous', 'noopUnmatched']
    )
    OR NOT public.affiliate_sync_v2_has_exact_keys(
      _counts->'offers',
      ARRAY['create', 'noopExisting', 'noopHeld', 'noopUnresolved']
    )
  THEN
    RETURN false;
  END IF;
  IF NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts#>'{stores,create}')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts#>'{stores,noopExisting}')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts#>'{stores,blockedAmbiguous}')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts#>'{stores,noopUnmatched}')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts#>'{offers,create}')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts#>'{offers,noopExisting}')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts#>'{offers,noopHeld}')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts#>'{offers,noopUnresolved}')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts->'writableStores')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts->'writableOffers')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts->'writableEntities')
  THEN
    RETURN false;
  END IF;
  RETURN (_counts#>>'{stores,blockedAmbiguous}')::integer = 0
    AND (_counts->>'writableStores')::integer = (_counts#>>'{stores,create}')::integer
    AND (_counts->>'writableOffers')::integer = (_counts#>>'{offers,create}')::integer
    AND (_counts->>'writableEntities')::integer =
      (_counts->>'writableStores')::integer + (_counts->>'writableOffers')::integer
    AND (_counts#>>'{stores,create}')::numeric
      + (_counts#>>'{stores,noopExisting}')::numeric <= 2147483647
    AND (_counts#>>'{offers,create}')::numeric
      + (_counts#>>'{offers,noopExisting}')::numeric
      + (_counts#>>'{offers,noopHeld}')::numeric
      + (_counts#>>'{offers,noopUnresolved}')::numeric <= 2147483647
    AND (_counts->>'writableStores')::numeric
      + (_counts->>'writableOffers')::numeric <= 2147483647
    AND (_counts#>>'{stores,create}')::numeric
...
CREATE OR REPLACE FUNCTION public.affiliate_sync_v2_valid_persistence_counts(
  _counts jsonb
)
RETURNS boolean
LANGUAGE plpgsql
...
BEGIN
  IF NOT public.affiliate_sync_v2_has_exact_keys(
    ...
    OR NOT public.affiliate_sync_v2_valid_expected_counts(_counts->'expected')
    OR NOT public.affiliate_sync_v2_has_exact_keys(
      _counts->'actual',
      ARRAY[
        'storesCreated',
  ...
  THEN
    RETURN false;
  END IF;
  IF NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts#>'{actual,storesCreated}')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts#>'{actual,storesNoopExisting}')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts#>'{actual,offersCreated}')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts#>'{actual,offersNoopExisting}')
    OR NOT public.affiliate_sync_v2_is_nonnegative_integer(_counts#>'{actual,ledgerRows}')
  THEN
    RETURN false;
  END IF;
  RETURN (_counts#>>'{actual,ledgerRows}')::integer =
      (_counts#>>'{actual,storesCreated}')::integer
      + (_counts#>>'{actual,storesNoopExisting}')::integer
      + (_counts#>>'{actual,offersCreated}')::integer
      + (_counts#>>'{actual,offersNoopExisting}')::integer
    AND (_counts#>>'{actual,storesCreated}')::integer
      + (_counts#>>'{actual,storesNoopExisting}')::integer =
      (_counts#>>'{expected,stores,create}')::integer
      + (_counts#>>'{expected,stores,noopExisting}')::integer
    AND (_counts#>>'{actual,offersCreated}')::integer
      + (_counts#>>'{actual,offersNoopExisting}')::integer =
      (_counts#>>'{expected,offers,create}')::integer
      + (_counts#>>'{expected,offers,noopExisting}')::integer
    AND (_counts#>>'{actual,storesCreated}')::integer <=
      (_counts#>>'{expected,stores,create}')::integer
    AND (_counts#>>'{actual,storesNoopExisting}')::integer >=
      (_counts#>>'{expected,stores,noopExisting}')::integer
    AND (_counts#>>'{actual,offersCreated}')::integer <=
      (_counts#>>'{expected,offers,create}')::integer
    AND (_counts#>>'{actual,offersNoopExisting}')::integer >=
      (_counts#>>'{expected,offers,noopExisting}')::integer;
EXCEPTION WHEN OTHERS THEN
  RETURN false;
END;
...
REVOKE ALL ON FUNCTION public.affiliate_sync_v2_has_exact_keys(jsonb, text[])
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.affiliate_sync_v2_is_nonnegative_integer(jsonb)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.affiliate_sync_v2_is_iso_date(text)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.affiliate_sync_v2_is_canonical_provider_id(text)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.affiliate_sync_v2_valid_expected_counts(jsonb)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.affiliate_sync_v2_valid_persistence_counts(jsonb)
  FROM PUBLIC, anon, authenticated;

GRANT EXECUTE ON FUNCTION public.affiliate_sync_v2_has_exact_keys(jsonb, text[])
  TO service_role;
GRANT EXECUTE ON FUNCTION public.affiliate_sync_v2_is_nonnegative_integer(jsonb)
  TO service_role;
GRANT EXECUTE ON FUNCTION public.affiliate_sync_v2_is_iso_date(text)
  TO service_role;
GRANT EXECUTE ON FUNCTION public.affiliate_sync_v2_is_canonical_provider_id(text)
  TO service_role;
GRANT EXECUTE ON FUNCTION public.affiliate_sync_v2_valid_expected_counts(jsonb)
  TO service_role;
GRANT EXECUTE ON FUNCTION public.affiliate_sync_v2_valid_persistence_counts(jsonb)
  TO service_role;

ALTER TABLE public.affiliate_import_runs
  ADD COLUMN persistence_contract_version text,
  ADD COLUMN plan_fingerprint_algorithm text,
  ADD COLUMN plan_fingerprint text,
  ADD COLUMN plan_evaluated_at timestamptz,
  ADD COLUMN persistence_execution_status text,
  ADD COLUMN persistence_counts jsonb;

ALTER TABLE public.affiliate_import_runs
  ADD CONSTRAINT affiliate_import_runs_v2_persistence_coherence_check
  CHECK (
    (
    ...
    (
      ...
      AND persistence_contract_version = 'v2-a9b-1'
      AND plan_fingerprint_algorithm = 'sha256-canonical-plan-v1'
      AND plan_fingerprint ~ '^[0-9a-f]{64}$'
      AND plan_evaluated_at IS NOT NULL
      AND persistence_execution_status IS NOT DISTINCT FROM 'committed'
      AND public.affiliate_sync_v2_valid_persistence_counts(persistence_counts)
      AND provider = 'impact'
      AND preview = false
      AND success = true
...
CREATE UNIQUE INDEX affiliate_import_runs_v2_replay_uidx
  ON public.affiliate_import_runs (
    provider,
    integration_id,
  ...
  WHERE preview = false
    AND persistence_contract_version IS NOT NULL;

CREATE TABLE public.affiliate_import_run_mutations_v2 (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  run_id uuid NOT NULL,
  instruction_ordinal integer NOT NULL,
  ...
  CONSTRAINT affiliate_import_run_mutations_v2_pkey PRIMARY KEY (id),
  CONSTRAINT affiliate_import_run_mutations_v2_run_fkey
    FOREIGN KEY (run_id)
    REFERENCES public.affiliate_import_runs(id)
    ON DELETE RESTRICT,
  CONSTRAINT affiliate_import_run_mutations_v2_ordinal_nonnegative_check
    CHECK (instruction_ordinal >= 0),
  CONSTRAINT affiliate_import_run_mutations_v2_entity_kind_check
    CHECK (entity_kind IN ('store', 'offer')),
  CONSTRAINT affiliate_import_run_mutations_v2_planned_action_check
    CHECK (planned_action IN ('create', 'noop_existing')),
  CONSTRAINT affiliate_import_run_mutations_v2_outcome_check
    CHECK (outcome IN ('created', 'noop_existing')),
  CONSTRAINT affiliate_import_run_mutations_v2_provider_check
    CHECK (provider = 'impact'),
  CONSTRAINT affiliate_import_run_mutations_v2_provider_entity_check
    CHECK (public.affiliate_sync_v2_is_canonical_provider_id(provider_entity_id)),
  CONSTRAINT affiliate_import_run_mutations_v2_action_outcome_check
    CHECK (
      (
      ...
      (
        ...
        AND outcome IN ('created', 'noop_existing')
      )
    ),
  CONSTRAINT affiliate_import_run_mutations_v2_entity_shape_check
    CHECK (
      (
        entity_kind = 'store'
        AND parent_provider_entity_id IS NULL
      ...
      (
        entity_kind = 'offer'
        AND parent_provider_entity_id IS NOT NULL
        AND public.affiliate_sync_v2_is_canonical_provider_id(parent_provider_entity_id)
        AND parent_entity_id IS NOT NULL
        AND offer_kind IS NOT NULL
        AND offer_kind IN ('coupon', 'deal')
      )
    ),
  CONSTRAINT affiliate_import_run_mutations_v2_run_ordinal_key
    UNIQUE (run_id, instruction_ordinal),
  CONSTRAINT affiliate_import_run_mutations_v2_run_identity_key
    UNIQUE (run_id, entity_kind, provider, provider_entity_id),
  CONSTRAINT affiliate_import_run_mutations_v2_run_entity_key
    UNIQUE (run_id, entity_kind, entity_id)
);

ALTER TABLE public.affiliate_import_run_mutations_v2 ENABLE ROW LEVEL SECURITY;

REVOKE ALL ON TABLE public.affiliate_import_run_mutations_v2
  FROM PUBLIC, anon, authenticated, service_role;
GRANT SELECT ON TABLE public.affiliate_import_run_mutations_v2
  TO authenticated, service_role;

CREATE POLICY "Admins read V2 import mutation evidence"
  ON public.affiliate_import_run_mutations_v2
  FOR SELECT
  TO authenticated
