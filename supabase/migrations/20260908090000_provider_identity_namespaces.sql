-- Provider-identity namespace cutover for the existing Promotions V2
-- persistence contract and the future exact Ads identity. V1 execution must
-- be frozen before this migration is applied. This migration does not add Ads
-- persistence or replace the historical V1 import_apply function.

-- Only four live Impact store identities have been proven by exact Campaign
-- lookup. Abort before any namespace DDL if another complete Impact identity
-- exists; never infer Campaign identity from provider, name, slug, URL, or
-- metadata alone.
DO $provider_identity_backfill_guard$
BEGIN
  IF EXISTS (
    SELECT 1
    FROM public.stores
    WHERE provider = 'impact'
      ...
      AND NOT (
        provider_entity_id = ANY (
          ARRAY['15781', '17513', '18744', '25810']::text[]
        )
      )
  ) THEN
    RAISE EXCEPTION 'unexpected_existing_impact_store_identity'
      USING ERRCODE = '23514';
  END IF;
END;
$provider_identity_backfill_guard$;

ALTER TABLE public.stores
  ADD COLUMN provider_entity_namespace text;

ALTER TABLE public.coupons
  ADD COLUMN provider_entity_namespace text;

-- Assign Campaign identity only to the four exact production CampaignIds.
UPDATE public.stores
SET provider_entity_namespace = 'campaign'
WHERE provider = 'impact'
  AND provider_entity_id = ANY (
    ARRAY['15781', '17513', '18744', '25810']::text[]
  );

-- Historical non-Impact provider identities remain explicitly unproven.
UPDATE public.stores
SET provider_entity_namespace = 'legacy'
WHERE provider IS NOT NULL
  AND provider <> 'impact'
  AND provider_entity_id IS NOT NULL;

-- No provider-backed coupons exist in production. Historical provider-backed
-- coupon fixtures cannot be proven Promotion or Ad identities and remain legacy.
...
ALTER TABLE public.stores
  ADD CONSTRAINT stores_provider_entity_identity_check
  CHECK (
    (
      provider IS NULL
    ...
    (
      ...
      AND public.affiliate_sync_v2_is_canonical_provider_id(provider)
      AND provider_entity_namespace IS NOT NULL
      AND public.affiliate_sync_v2_is_canonical_provider_id(
        provider_entity_namespace
      )
      AND provider_entity_id IS NOT NULL
      AND public.affiliate_sync_v2_is_canonical_provider_id(provider_entity_id)
      AND (
        (provider = 'impact' AND provider_entity_namespace IN ('campaign', 'legacy'))
        OR (provider <> 'impact' AND provider_entity_namespace = 'legacy')
      )
    )
  );

ALTER TABLE public.coupons
  ADD CONSTRAINT coupons_provider_entity_identity_check
  CHECK (
    (
      provider IS NULL
      AND provider_entity_namespace IS NULL
...
DROP INDEX public.stores_provider_entity_uidx;
DROP INDEX public.coupons_provider_entity_uidx;

CREATE UNIQUE INDEX stores_provider_entity_uidx
  ON public.stores (provider, provider_entity_namespace, provider_entity_id)
  WHERE provider IS NOT NULL
    AND provider_entity_namespace IS NOT NULL
    AND provider_entity_id IS NOT NULL;

CREATE UNIQUE INDEX coupons_provider_entity_uidx
  ON public.coupons (provider, provider_entity_namespace, provider_entity_id)
  WHERE provider IS NOT NULL
    AND provider_entity_namespace IS NOT NULL
    AND provider_entity_id IS NOT NULL;

-- Historical flat-contract rows remain internally valid evidence, but only the
-- replacement RPC below can create/replay the namespace-aware next contract.
ALTER TABLE public.affiliate_import_runs
  DROP CONSTRAINT affiliate_import_runs_v2_persistence_coherence_check,
  ...
  CHECK (
    ...
    (
      ...
      AND persistence_contract_version IN ('v2-a9b-1', 'v2-a9b-2')
...
ALTER TABLE public.affiliate_import_run_mutations_v2
  ADD COLUMN provider_entity_namespace text,
  ADD COLUMN parent_provider_entity_namespace text;

-- Production ledger is empty. These deterministic values also preserve
-- disposable fixtures created by the existing Promotions V2 contract.
UPDATE public.affiliate_import_run_mutations_v2
SET provider_entity_namespace = CASE
      WHEN entity_kind = 'store' THEN 'campaign'
      ELSE 'promotion'
    END,
    parent_provider_entity_namespace = CASE
      WHEN entity_kind = 'offer' THEN 'campaign'
      ELSE NULL
    END;

ALTER TABLE public.affiliate_import_run_mutations_v2
  ALTER COLUMN provider_entity_namespace SET NOT NULL,
  DROP CONSTRAINT affiliate_import_run_mutations_v2_provider_entity_check,
  DROP CONSTRAINT affiliate_import_run_mutations_v2_entity_shape_check,
  DROP CONSTRAINT affiliate_import_run_mutations_v2_run_identity_key,
  ADD CONSTRAINT affiliate_import_run_mutations_v2_provider_entity_check
    CHECK (
      public.affiliate_sync_v2_is_canonical_provider_id(provider_entity_namespace)
      AND public.affiliate_sync_v2_is_canonical_provider_id(provider_entity_id)
    ),
  ADD CONSTRAINT affiliate_import_run_mutations_v2_entity_shape_check
    CHECK (
      (
        entity_kind = 'store'
        AND provider_entity_namespace = 'campaign'
        AND parent_provider_entity_namespace IS NULL
  ...
  ADD CONSTRAINT affiliate_import_run_mutations_v2_run_identity_key
    UNIQUE (
      run_id,
      entity_kind,
      provider,
...
BEGIN
  ...
  BEGIN
    IF _integration_id IS NULL
      ...
      OR _persistence_contract_version IS DISTINCT FROM 'v2-a9b-2'
    ...
    LOOP
      IF NOT public.affiliate_sync_v2_has_exact_keys(
        ...
        ARRAY[
          ...
          'parentProviderEntityId', 'expectedParentStoreId', 'projection'
        ]
      )
        OR NOT public.affiliate_sync_v2_is_nonnegative_integer(v_instruction->'instructionOrdinal')
        ...
        OR jsonb_typeof(v_instruction->'providerEntityNamespace')
          IS DISTINCT FROM 'string'
        OR v_instruction->>'providerEntityNamespace' IS DISTINCT FROM 'promotion'
        OR jsonb_typeof(v_instruction->'providerEntityId') IS DISTINCT FROM 'string'
      ...
      SELECT coalesce(jsonb_agg(
        jsonb_build_object(
          ...
          'parentProviderEntityNamespace',
            mutation.parent_provider_entity_namespace,
    ...
    LOOP
      ...
      WHERE store.provider = 'impact'
        AND store.provider_entity_namespace = v_provider_entity_namespace
    ...
    LOOP
      ...
      WHERE offer.provider = 'impact'
        AND offer.provider_entity_namespace = v_provider_entity_namespace
    ...
    LOOP
      INSERT INTO public.affiliate_import_run_mutations_v2 (
        ...
        entity_id,
        expected_entity_id,
        parent_provider_entity_namespace,
