...
function headerInteger(headers: Headers, names: readonly string[]): {
  present: boolean;
  value: number | null;
} {
  let present = false;
...
/** Host-only bounded Impact transport; Basic credentials stay inside the shared adapter. */
export class ImpactAdsApplyTransportHost implements ImpactAdsTransportV2 {
    ...
    this.inner = new ImpactTransportHost({
      ...
      fetchImplementation: async (request, init) => {
        ...
        try {
          ...
          throw new Error("ads_apply_bounded_body_read_failed");
        }
      },
    });
  }
  ...
  private observeRate(headers: Headers): void {
    for (const key of ["limit", "remaining", "reset"] as const) {
      const observed = headerInteger(headers, RATE_HEADERS[key]);
  ...
  consumeResponseSizeLimitExceeded(): boolean {
    const result = this.responseSizeLimitExceeded;
    this.responseSizeLimitExceeded = false;
    return result;
  }
}
