...
test("apply config fixes the global source to exact Coupon Ads and trusted Campaigns", () => {
  const resolved = resolveImpactAdsApplyHostConfigV2(integration, credentials);
...
test("apply config rejects alternate Ads inputs and untrusted Campaign resources", () => {
  for (
    const campaigns of [
    ...
    assert.throws(() =>
      resolveImpactAdsApplyHostConfigV2({
        ...integration,
  ...
  // Stored Ads-like fields cannot alter the hard-coded global source.
  const resolved = resolveImpactAdsApplyHostConfigV2({
    ...integration,
    endpointConfiguration: {
      ...integration.endpointConfiguration,
      ads: "https://evil.example/Ads?Type=COUPON",
      promotions: "/Mediapartners/{AccountSID}/Promotions",
    },
  }, credentials);
  assert.equal(
    new URL(resolved.adsInitialUrl).origin,
    "https://api.impact.com",
  );
  assert.equal(
    new URL(resolved.adsInitialUrl).pathname,
    "/Mediapartners/account-id/Ads",
  );
});

test("apply config rejects unsafe provider, auth and base URL settings", () => {
  for (
    const patch of [
      { baseUrl: "http://api.impact.com" },
      { baseUrl: "https://api.impact.com/path" },
      { baseUrl: "https://user:pass@api.impact.com" },
      { baseUrl: "https://api.impact.com?secret=value" },
      { providerName: "other" },
...
test("apply config retains bounded fallback and clamping semantics", () => {
  const resolved = resolveImpactAdsApplyHostConfigV2({
    ...integration,
    timeoutSeconds: 999,
    retryAttempts: 99,
