...
import { ENRICHMENT_KEY, readEnrichment, type OfferEnrichment } from "../../enrichment/types.ts";
import type {
  CanonicalCategory,
  CanonicalCoupon,
  CanonicalDeal,
...
export class ImpactNormalizer extends BaseNormalizer {
  ...
  normalizeCoupon(raw: unknown, ctx?: NormalizerContext): StandardResponse<CanonicalCoupon> {
    ...
    if (!id) return normalizerFail(PROVIDER, "Coupon record is missing PromotionIds", 0, ctx?.integrationId);
