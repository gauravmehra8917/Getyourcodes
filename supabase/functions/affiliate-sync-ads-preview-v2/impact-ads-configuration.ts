...
import type { ResolvedImpactAdsHostConfigV2 } from "./types.ts";
...
function resolveCampaignsUrl(
  endpointConfiguration: Record<string, unknown>,
  base: URL,
  accountSid: string,
  pageSize: number,
): string {
  const endpoint = stripGetPrefix(
    configuredCampaignEndpoint(endpointConfiguration),
  );
  ...
  try {
    pathSegments = url.pathname.split("/").filter(Boolean).map(
      decodeURIComponent,
    );
  } catch {
    throw new HostConfigurationError("invalid_integration_config");
  ...
  if (
    pathSegments.length !== 3 || pathSegments[0] !== "Mediapartners" ||
    pathSegments[1] !== accountSid || pathSegments[2] !== "Campaigns"
  ) throw new HostConfigurationError("invalid_integration_config");
  ...
  const pageEntries = [...url.searchParams.entries()].filter(([key]) =>
    key.toLowerCase() === "page"
  );
  const pageSizeEntries = [...url.searchParams.entries()].filter(([key]) =>
    key.toLowerCase() === "pagesize"
  );
  if (
    pageEntries.length !== 1 || pageEntries[0]?.[0] !== "Page" ||
    pageEntries[0]?.[1] !== "1" || pageSizeEntries.length !== 1 ||
    pageSizeEntries[0]?.[0] !== "PageSize" ||
    pageSizeEntries[0]?.[1] !== String(pageSize)
  ) throw new HostConfigurationError("invalid_integration_config");
  return url.toString();
}

function resolveAdsUrl(base: URL, accountSid: string): string {
  const url = new URL(
...
export function resolveImpactAdsHostConfigV2(
  ...
  return {
    baseUrl: base.origin,
    adsInitialUrl: resolveAdsUrl(base, credentials.accountSid),
    campaignsInitialUrl: resolveCampaignsUrl(
