...
# where psql connects over the container Unix socket.
test_db=${1:-}
db_user=${A11_I2_DB_USER:-supabase_admin}

if [[ ! "$test_db" =~ ^a11_i2_[a-z0-9_]+$ ]]; then
  echo "expected disposable database name matching a11_i2_[a-z0-9_]+" >&2
  exit 2
fi

script_dir=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
fixture_path="$script_dir/affiliate_sync_ads_v2_contention_fixture.psql"
...
if [[ "$("${psql_base[@]}" -c "select current_database() = '$test_db' and current_database() like 'a11_i2_%' and inet_server_addr() is null")" != "t" ]]; then
  echo "refusing non-disposable or TCP-connected database" >&2
  exit 2
fi

temp_root=$(mktemp -d /tmp/a11-i2-contention.XXXXXX)
cleanup() {
  rm -rf -- "$temp_root"
}
trap cleanup EXIT
...
run_pair() {
  ...
  local winner_app="a11_i2_${app_case}_a"
  local contender_app="a11_i2_${app_case}_b"
...
BEGIN;
WITH response AS (
  SELECT a11_i2_contention.execute_request('$case_id', 'A') AS result
)
INSERT INTO a11_i2_contention.results (case_id, side, result)
...
SET ROLE service_role;
WITH response AS (
  SELECT a11_i2_contention.execute_request('$case_id', 'B') AS result
)
INSERT INTO a11_i2_contention.results (case_id, side, result)
...
run_legacy_pair() {
  local case_id=$1
  local app_case=${case_id//-/_}
  local winner_app="a11_i2_${app_case}_legacy_writer"
  local contender_app="a11_i2_${app_case}_rpc"
  local winner_sql="$temp_root/${case_id}-legacy.psql"
  local contender_sql="$temp_root/${case_id}-rpc.psql"
  local winner_out="$temp_root/${case_id}-legacy.out"
  local winner_err="$temp_root/${case_id}-legacy.err"
  local contender_out="$temp_root/${case_id}-rpc.out"
  local contender_err="$temp_root/${case_id}-rpc.err"

  if [[ "$case_id" == "legacy-store-race" ]]; then
    cat > "$winner_sql" <<SQL
SET application_name = '$winner_app';
SET statement_timeout = '20s';
SET lock_timeout = '12s';
...
INSERT INTO public.stores (
  ...
  'Concurrent legacy Campaign', 'concurrent-legacy-campaign',
  'impact', 'legacy', 'campaign-legacy-race'
);
SELECT pg_sleep(3);
COMMIT;
SQL
...
SELECT id, 'Concurrent legacy Ad', 'impact', 'legacy', 'ad-legacy-race'
FROM public.stores
WHERE provider = 'impact'
  AND provider_entity_namespace = 'campaign'
  AND provider_entity_id = 'campaign-parent-a';
SELECT pg_sleep(3);
COMMIT;
SQL
  fi
...
SQL
  ...
  if ! wait_for_sql_true "select exists (select 1 from pg_stat_activity where application_name = '$winner_app' and wait_event_type = 'Timeout' and wait_event = 'PgSleep')"; then
    wait "$winner_process" || true
    cat "$winner_err" >&2
    echo "$case_id: legacy writer never reached the transaction hold" >&2
    exit 1
  fi
  ...
  if ! wait_for_sql_true "select coalesce((select wait_event_type = 'Lock' and wait_event = 'transactionid' and cardinality(pg_blocking_pids(pid)) > 0 from pg_stat_activity where application_name = '$contender_app'), false)"; then
    ...
    cat "$winner_err" >&2
    cat "$contender_err" >&2
    echo "$case_id: RPC completed without proven namespace-index contention" >&2
    exit 1
  fi
  ...
  "${psql_base[@]}" -c "
    ...
    from contender
  " >> "$temp_root/legacy-lock-evidence.out"
...
for case_id in \
  ...
  different-offers parent-conflict slug-conflict
do
  run_pair "$case_id"
done

for case_id in legacy-store-race legacy-offer-race; do
  run_legacy_pair "$case_id"
done

validation_result=$("${psql_base[@]}" -c "select a11_i2_contention.validate_results()")
if [[ "$validation_result" != "A11-S4-I2 Ads contention matrix passed" ]]; then
...
while IFS= read -r evidence; do
  echo "LEGACY_LOCK_EVIDENCE|$evidence"
done < "$temp_root/legacy-lock-evidence.out"
"${psql_base[@]}" -c "
  select concat_ws('|',
    case_id,
    side,
