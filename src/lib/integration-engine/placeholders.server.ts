// Generic endpoint variable resolution.
//
// Resolves `{Placeholder}` tokens found in endpoint paths, query values and
// header values from the integration's credentials / config. Fully
// provider-agnostic: matching is normalized (case-insensitive, non-alphanumeric
// characters stripped) so `{AccountSID}`, `{account_sid}` and `{accountId}`
// all map to the same credential value.

import type { IntegrationConfig, IntegrationCredentials } from "./types";

/** Canonical variable name -> list of accepted aliases (normalized). */
...
export interface ResolveResult {
  ...
  resolved: string[];
  /** Placeholder names with no matching credential (left untouched). */
  unresolved: string[];
}
