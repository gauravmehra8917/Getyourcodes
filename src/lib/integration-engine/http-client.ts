// Runtime-neutral HTTP client. Hosts can optionally provide a write-capable
// request-log sink; the Edge preview intentionally does not.

import { buildRequest } from "./request-builder";
import { logDebug } from "./logger";
import { shouldRetry, sleep } from "./retry-engine";
import { buildStandardResponse, classifyStatus, extractRateLimit, headersToObject } from "./response-handler";
import type { ErrorClass, HttpMethod, HttpRequestOptions, IntegrationConfig, StandardResponse } from "./types";

const DEFAULT_MAX_BYTES = 5 * 1024 * 1024;

export interface RequestLogEntry {
  integrationId: string; method: HttpMethod; url: string; status: number; latencyMs: number; retryCount: number;
  outcome: "success" | "failure"; errorClass?: string; message?: string; environment?: string;
}
export type RequestLogSink = (entry: RequestLogEntry, persist: boolean) => void;
...
export async function executeRequest<T = unknown>(config: IntegrationConfig, opts: HttpRequestOptions, logSink?: RequestLogSink): Promise<StandardResponse<T>> {
  ...
  const started = Date.now(); let attempt = 0; let lastResp: StandardResponse<T> | null = null;
  ...
  const finalResponse = lastResp as StandardResponse<T>;
  logSink?.({ integrationId: config.id, method: finalResponse.meta.method, url: finalResponse.meta.url, status: finalResponse.status, latencyMs: Date.now() - started, retryCount: finalResponse.retryCount, outcome: finalResponse.success ? "success" : "failure", errorClass: finalResponse.error?.class, message: finalResponse.error?.message, environment: config.environment }, !!opts.persistLog);
